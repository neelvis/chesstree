...
или `K+N vs K`, редьюсер завершает партию `TwoWayDraw`: эти игроки делят первое
место, а уже выбывший сохраняет третье. Реплей старой сохранённой истории, в
которой после такой позиции уже записаны ходы, остаётся допустимым; правило
срабатывает на следующем новом ходе и тем самым завершает устаревшую партию без
потери истории.
